...
class Node:
	...
	def __init__(self, currentBoard, turn = -1):
	...
	def selectNode(self):
		...
		if (len(self.unvisitedNodes) == 0):
			# find child with best UTC value
			bestUTCChild = self.getBestUCT();

			return bestUTCChild.selectNode();

		return self.expand();


	def makeNodeFromMove(self, move):
		return Node(makeMoveOther(self.board, move, self.turn), -self.turn);
	...
	def simulateGame(self):
		...
		if (self.isLeaf):
			return getState(self.board) * self.turn;

		randomMove = choice(self.possibleMoves);

		result = self.makeNodeFromMove(randomMove).simulateGame();
